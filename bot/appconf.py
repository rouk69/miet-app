...
FLAGS = {
    ...
    "orioks_open": (
        False,
        "Раздел «Учёба» открыт всем",
        "Иначе — только владельцу и тем, кому доступ выдан в карточке "
        "пользователя. Сдача работ остаётся только у владельца. "
        "Переключает только владелец",
    ),
    "raffle_on": (
        False,
        "Розыгрыш виден всем",
        "Пока выключено, раздел открывается только админам. Приглашения "
        "считаются в любом случае — к открытию таблица будет не пустой",
    ),
}
