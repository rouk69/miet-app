...
class SessionExpired(WebError):
    """
    Сессия больше не принимается — нужен новый вход.

    Отдельно от прочих ошибок намеренно: «ОРИОКС не отвечает» проходит
    само, а стирать из-за него сохранённый доступ значит просить
    человека вводить пароль всякий раз, когда у института учения.
    """
...
def study_json(cookie: str) -> dict:
    ...
    raw = _cut_json(html)
    if not raw:
        # Вместо данных пришла форма входа — значит сессия кончилась.
        if FIELD_PASSWORD in html:
